...
const LeadManagementView = ({ filters }) => {
  ...
  const updateColumnsFromPipeline = async (pipeline) => {
    ...
    try {
      const response = await fetch(
        "https://api.autopilotmybusiness.com/api/form/leads-by-stages"
      );
      ...
      const formattedColumns = pipeline.stages.map((stage) => {
        const matchingStage = leadsData.find(
          (lead) => lead.stage === stage.stageName
        );
  ...
  const moveContact = (contactId, fromColumnId, toColumnId) => {
    ...
    setColumns((prevColumns) => {
      ...
      const contactIndex = newColumns[fromColumnIndex].contacts.findIndex(
        (c) => c.id === contactId
      );
      if (contactIndex === -1) return prevColumns;

      const [movedContact] = newColumns[fromColumnIndex].contacts.splice(
        contactIndex,
        1
      );
      ...
      newColumns[fromColumnIndex].leads =
        newColumns[fromColumnIndex].contacts.length;
      newColumns[toColumnIndex].leads =
        newColumns[toColumnIndex].contacts.length;
              ...
              <div className="pipeline-column">
                ...
                {column.showMeta && (
                  <div className="stage-extras">
                    {column.metadata.what && (
                      <div className="meta-item">
                        <strong>What:</strong>{" "}
                        <span>{column.metadata.what}</span>
                      </div>
                    )}
                    {column.metadata.when && (
                      <div className="meta-item">
                        <strong>When:</strong>{" "}
                        <span>
                          {column.metadata.when}
                    ...
                    {column.metadata.who && (
                      <div className="meta-item">
                        <strong>Who:</strong> <span>{column.metadata.who.fullName}</span>
                      </div>
                    )}
                    {column.metadata.how && (
                      <div className="meta-item">
                        <strong>How:</strong> <span>{column.metadata.how}</span>
                      </div>
                    )}
                  </div>
                )}
                ...
                <div className="contacts-container">
                  <ContactCard
                    ...
                    onContactMove={(contactId) =>
                      moveContact(contactId, column.id)
                    }
                  />
                </div>
              </div>
