...
router.use((req, res, next) => {
  ...
  if (Object.keys(req.cookies).length > 0) {
    ...
    console.log(`🍪 [${requestId}] No cookies present`);
  }
  ...
  if (
    ...
    console.log(`📦 [${requestId}] No body or multipart content`);
  }
  ...
  console.log(`🔹 [${requestId}] Proceeding to next middleware`);
  next();
});
...
router.get("/auth/callback", async (req, res) => {
  ...
  try {
    ...
    if (!companyName) {
      console.log(`[${_requestId}] ❌ No company name found in token cookie`);
      return res.status(400).send("Company name required");
    }

    console.log(
      `[${_requestId}] 🔄 Processing auth callback for ${companyName}`
    );
    ...
    console.log(`[${_requestId}] 🔄 Creating session for user ${userId}`);
    // Get model dynamically based on company name
    ...
    console.log(`[${_requestId}] ✅ Session created for user ${userId}`);
    setAuthCookie(res, "session_id", sessionId, {
      maxAge: expires_in * 1000,
      path: "/api", // Ensure cookie is sent for API requests
    });
...
router.use(async (req, res, next) => {
  const { _requestId } = req;
  console.log(`[${_requestId}] 🚀 Starting authentication middleware`);

  try {
    console.log(`[${_requestId}] 🔍 Checking company name from cookies`);
    const companyName = getCompanyNameFromCookie(req);
    console.log(`[${_requestId}] ℹ️ Company name:`, companyName);

    if (!companyName) {
      console.log(`[${_requestId}] ⚠️ No company name found in cookies`);
      return res.status(401).json({ error: "Authentication required" });
    }

    console.log(`[${_requestId}] 🔍 Checking session_id from cookies`);
    const { session_id } = req.cookies;
    console.log(`[${_requestId}] ℹ️ Session ID:`, session_id);

    if (!session_id) {
      console.log(`[${_requestId}] ⚠️ No session_id found in cookies`);
      return res.status(401).json({ error: "Session expired" });
    }

    console.log(
      `[${_requestId}] 📦 Getting MicrosoftSession model for company:`,
      companyName
    );
    const MicrosoftSession = await getMicrosoftSessionModel(companyName);

    console.log(`[${_requestId}] 🔎 Searching for valid session in database`);
    const session = await MicrosoftSession.findOne({
      sessionId: session_id,
      expiresAt: { $gt: new Date() }, // Only valid sessions
    });

    console.log(`[${_requestId}] ℹ️ Found session:`, session);

    if (!session) {
      console.log(`[${_requestId}] ⚠️ No valid session found in database`);
      return res.status(401).json({ error: "Session expired" });
    }

    console.log(`[${_requestId}] ✅ Authentication successful`);
    req.session = session;
    req.companyName = companyName;
    next();
  } catch (err) {
    console.error(`[${_requestId}] ❌ Auth error:`, err);
    return res.status(500).json({ error: "Authentication failed" });
  }
});
// Upload File
router.post("/auth/upload", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    console.log(`[${_requestId}] 📤 Processing file upload for ${companyName}`);
...
// Get All Uploads
// Get All Uploads
router.get("/auth/uploads", async (req, res) => {
