  ...
  router.get("/user/:userId", async (req, res) => {
    ...
    try {
      ...
      const filteredPipelines = pipelines.map((pipeline) => {
        ...
        console.log(`➡️ Pipeline: ${pipeline.pipelineName} | Stages for user: ${userStages.length}`);
...
router.post("/add", async (req, res) => {
  try {
    ...
    if (!pipelineName || !stages.length) {
      return res.status(400).json({ message: "Pipeline name and at least one stage are required" });
    }
